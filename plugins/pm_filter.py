    ...
    if query.data.startswith("file"):
        ...
        try:
            if AUTH_CHANNEL and not await is_subscribed(client, query):
                ...
                await query.answer('මගෙ ඉන්බොක්ස් බලන්න ! මන් ෆයිල් එක දන්නම් !',show_alert = True)
        ...
        if AUTH_CHANNEL and not await is_subscribed(client, query):
            await query.answer("මරැ හැබැයි 😒",show_alert=True)
            return
        ident, file_id = query.data.split("#")
        files = (await get_file_details(file_id))[0]
        title = files.file_name
        ...
        reply_markup = InlineKeyboardMarkup(buttons)
        await query.message.edit_text(
            text=script.HOWTOFIND_TXT,
            reply_markup=reply_markup,
            parse_mode=enums.ParseMode.HTML
        )
    elif query.data == "grupak":
        buttons = [[
            InlineKeyboardButton('👩‍🦯 Back', callback_data='howtofind')
        ]]
        reply_markup = InlineKeyboardMarkup(buttons)
        await query.message.edit_text(
            text=script.GRUPAK_TXT,
            reply_markup=reply_markup,
            parse_mode=enums.ParseMode.HTML
        )
    elif query.data == "howtogive":
        buttons = [[
            InlineKeyboardButton('👩‍🦯 Back', callback_data='help')
        ]]
        reply_markup = InlineKeyboardMarkup(buttons)
        await query.message.edit_text(
            text=script.HOWTOGIVE_TXT,
            reply_markup=reply_markup,
            parse_mode=enums.ParseMode.HTML
        )
